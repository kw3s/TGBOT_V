...
async function handleAudioMode(bot, msg) {
    ...
    if (msg.audio) {
        ...
        await bot.sendMessage(chatId, "Searching for Audio...");
        ...
        try {
            ...
            const sanitizedTitle = trackTitle.replace(/[<>:`"/\|?*]/g, '').substring(0, 100);
    ...
    // Default Instruction
    await bot.sendMessage(chatId, "🎵 Audio Only Mode\n\nPlease send me an Audio file (MP3/M4A). I will automatically find the cover art.\n\n💡 Don't have an audio file? Send a Spotify, Deezer, Tidal, Apple Music link to @MUSICSHUNTERSBOT, and forward the resulting audio here!");
}
