...
async function handleSearchMode(bot, msg) {
    ...
    try {
        ...
        if (directUrl) {
            ...
            const deemixResult = await downloadFromDeemix(query);

            if (deemixResult && deemixResult.filePath) {
                ...
                // We need to signal to skip yt-dlp download
                meta = {
                    title: deemixResult.title,
                    filePath: deemixResult.filePath,
                    extractor: 'deemix'
                };
            } else {
